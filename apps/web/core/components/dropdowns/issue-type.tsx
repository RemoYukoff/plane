...
type Props = TDropdownProps & {
  ...
  /** Epics are a type like any other, but picking one is a different intent. */
...
export const IssueTypeDropdown = observer(function IssueTypeDropdown(props: Props) {
  ...
  // derived values
  const issueTypes = getProjectIssueTypes(projectId).filter((issueType) => includeEpics || !issueType.is_epic);
      ...
      {button ? (
        ...
        >
          <DropdownButton
            ...
            tooltipContent={selectedIssueType ? selectedIssueType.name : placeholder}
            showTooltip={showTooltip}
            variant={buttonVariant}
            renderToolTipByDefault={renderByDefault}
          >
            {!hideIcon && selectedIssueType && <Logo logo={selectedIssueType.logo_props} size={12} />}
            {(selectedIssueType || placeholder) && BUTTON_VARIANTS_WITH_TEXT.includes(buttonVariant) && (
              <span className="truncate">
                {selectedIssueType ? selectedIssueType.name : <span className="text-placeholder">{placeholder}</span>}
              </span>
            )}
            {dropdownArrow && (
