...
export class IssueTypeService extends APIService {
  ...
  /**
   * Removes the type from this project. Types are workspace owned, so this
   * unlinks it here rather than deleting it everywhere.
   */
  async removeIssueTypeFromProject(workspaceSlug: string, projectId: string, issueTypeId: string): Promise<void> {
  ...
  /** Links existing workspace level types into a project. */
  async importIssueTypes(workspaceSlug: string, projectId: string, issueTypeIds: string[]): Promise<TIssueType[]> {
    return this.post(`/api/workspaces/${workspaceSlug}/projects/${projectId}/import-issue-types/`, {
